...
class CarRacing(gym.Env, EzPickle):
    ...
    def _init_colors(self):
        if self.domain_randomize:
            ...
            self.middle_line_color = np.array([255, 0, 255])
    ...
    def step(self, action: Union[np.ndarray, int]):
        ...
        self.state, self.argu_img = self._render("state_pixels")
        ...
        info = {
            "augmented_img": self.argu_img,
            "car_position_vector": self.car.hull.position,
    ...
    def _render(self, mode: str):
        ...
        if self.MODE:
            self._render_addline(self.MODE, zoom, trans, angle)
        self.car.draw(
            self.surf,
            zoom,
            trans,
        ...
        if mode == "rgb_array":
            ...
            # arr_augm = self.get_arr_augm_from_surf()
            return self._create_image_array(
                self.surf, (STATE_W, STATE_H)
    ...
    def _render_addline(self, MODE, zoom, translation, angle):
        if MODE == "left":
            self._draw_addline(
                self.t2,
                color=(0, 255, 255),
    ...
    def _draw_addline(
        ...
        if zoom is None or translation is None or angle is None:
            raise ValueError("must input zoom/translation/angle")

        t = [pygame.math.Vector2(c).rotate_rad(angle) for c in t]
        t = [(c[0] * zoom + translation[0], c[1] * zoom + translation[1]) for c in t]
        ...
        if not clip or any(
            ...
            pygame.draw.lines(self.surf, color, False, t, line_width)
    ...
    def create_video(self, video_name="output.avi"):
        import cv2

        fourcc = cv2.VideoWriter_fourcc(*"XVID")
        out = cv2.VideoWriter(video_name, fourcc, 20.0, (WINDOW_W, WINDOW_H))

        for frame in self.frames:
            flipped_frame = cv2.flip(frame, 0)
            out.write(cv2.cvtColor(flipped_frame, cv2.COLOR_RGB2BGR))
    ...
    def return_velocity_vector(self):
        linear_velocity = self.car.hull.linearVelocity
